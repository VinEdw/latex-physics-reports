...
Table \ref{tab:long_fiber_lengths} contains the calculated lengths of the long fiber optic cable ($L$) used for each trial.
For trials 1 through 4, a roughly 20 m cable was used.
For trials 5 through 7, the length was reduced each trial to 10 m, 1 m, and 0.5 m in an attempt to get the infrared light to pass through the cable without dissipating.

\begin{table}[htbp]
\caption{\label{tab:long_fiber_lengths}
Calculated Long Fiber Optic Cable Length
}
...
\begin{IEEEeqnarray}{rCl}
  L & = & \frac{M}{\mu} \\
    & = & \frac{Ml}{m} \label{eq:long_cable_length}
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
  v & = & \frac{L}{t} \label{eq:distance_over_time} \\
  \frac{c}{n} & = & \frac{Ml/m}{t} \\
  c & = & \frac{nMl}{tm} \label{eq:speed_of_light}
\end{IEEEeqnarray}
The uncertainty in $c$ is given by equation \ref{eq:speed_of_light_uncertainty}.
\begin{IEEEeqnarray}{rCl}
  \Delta c & = & \left[
     \left(\frac{\partial c}{\partial l} \Delta l \right)^2 +
     \left(\frac{\partial c}{\partial t} \Delta t \right)^2 +
     \left(\frac{\partial c}{\partial M} \Delta M \right)^2 +
     \left(\frac{\partial c}{\partial m} \Delta m \right)^2
  \right]^\frac{1}{2} \\
  \Delta c & = & \frac{nMl}{tm} \left[
     \left(\frac{\Delta l}{l} \right)^2 +
     \left(\frac{\Delta t}{t} \right)^2 +
     \left(\frac{\Delta M}{M} \right)^2 +
     \left(\frac{\Delta m}{m} \right)^2
  \right]^\frac{1}{2} \label{eq:speed_of_light_uncertainty}
\end{IEEEeqnarray}
