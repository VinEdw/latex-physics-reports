...
For each coil of wire, resistance was determined in three ways: based on measurements of voltage and current; based on the resistance readings from the multimeter; and based on the dimensions of the wires and the resistivity of the material.
...
\br
  Trial & \mch{$\Delta V$ (V)} & \mch{$I$ (mA)}   & \mch{$R_\mathrm{m} + r$ (Ω)} \\
\mr
...
\br
  Trial & \mch{$R_{V/I}$ (Ω)} & \mch{$R_{\mathrm{m}}$ (Ω)} & \multicolumn{2}{l}{$R_{\mathrm{th}}$ (Ω)} \\
\mr
